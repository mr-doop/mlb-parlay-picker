...
def _home_from_gid(game_id: str) -> str:
    try:
        return _team_key(game_id.split("-")[-2])  # ← home is penultimate token
    except Exception:
        return ""
...
BALLPARKS: Dict[str, Dict] = {
    ...
    "ChicagoWhiteSox":{"lat":41.83,"lon":-87.634,"cf_bearing":5,"roof":False},
    ...
    "DetroitTigers":{"lat":42.339,"lon":-83.0485,"cf_bearing":5,"roof":False},
    ...
    "MilwaukeeBrewers":{"lat":43.028,"lon":-87.9712,"cf_bearing":8,"roof":True},
    ...
    "OaklandAthletics":{"lat":37.7516,"lon":-122.2,"cf_bearing":10,"roof":False},
    ...
    "WashingtonNationals":{"lat":38.873,"lon":-77.0074,"cf_bearing":7,"roof":False},
}
...
def _fetch_hourly(lat: float, lon: float, date: str) -> pd.DataFrame:
    ...
    h = j["hourly"]; df = pd.DataFrame(h); df["time"] = pd.to_datetime(df["time"])
    return df
