...
def fetch_dk_odds(date: str):
    ...
    params = {"apiKey": key, "regions": "us", "markets": "h2h,spreads,player_pitcher_strikeouts,player_pitcher_outs,player_pitcher_walks,player_pitcher_to_record_a_win", "oddsFormat": "american", "bookmakers": "draftkings"}
            ...
            for m in bm.get("markets",[]):
                ...
                if key=="h2h":
                    ...
                    for out in m.get("outcomes",[]):
                        ...
                        "player_pitcher_strikeouts","player_strikeouts","pitcher_strikeouts",
                        "player_pitcher_outs","pitcher_outs",
                        "player_pitcher_walks","pitcher_walks",
                        "player_pitcher_to_record_a_win","player_to_record_a_win","to_record_a_win"]):
                    for out in m.get("outcomes", []):
                        desc = str(out.get("description","")).lower()
                        name = str(out.get("name",""))
                        price = int(out.get("price",0))
                        point = out.get("point", None)
                        if any(s in key for s in ["strikeouts","player_pitcher_strikeouts","player_strikeouts","pitcher_strikeouts"]):
                        ...
                        elif any(s in key for s in ["outs","player_pitcher_outs","pitcher_outs"]):
                            side = "OVER" if "over" in name.lower() else ("UNDER" if "under" in name.lower() else "")
                            rows.append(DKRow(date,gid,"PITCHER_OUTS",side,"",name,name, str(point) if point is not None else "", price))
                        elif any(s in key for s in ["walks","player_pitcher_walks","pitcher_walks"]):
                            side = "OVER" if "over" in name.lower() else ("UNDER" if "under" in name.lower() else "")
                            rows.append(DKRow(date,gid,"PITCHER_WALKS",side,"",name,name, str(point) if point is not None else "", price))
                        elif any(s in key for s in ["player_pitcher_to_record_a_win","player_to_record_a_win","to_record_a_win"]):
